...
This app is a copy of the **retail** (Forma Lux) industry vertical, renamed **cbh** for a separate rendering and editing host. It uses the same patterns: Tailwind-based styling, modular components, and localization. Point it at the same or a different Sitecore site using `.env.local` as needed.
...
6. Install dependencies:
   from `industry-verticals\cbh` run `npm install`
...
6. Provide Editing host name `cbh` as per xmcloud.build.json
...
#### By default, this site supports the following locales:
